...
fn parse_input(s: DayString) -> Tower {
    ...
    fn initialise_disk(
        ...
        if let Entry::Occupied(entry) = un_disks.entry(name) {
            ...
            let above = children.iter().filter_map(|child| initialise_disk(child, un_disks, fin_disks)).collect();
            return Some(Disk { name, weight, above });
        }
        
        None
    }

    let re = Regex::new(r"(?m)^(?P<name>\w+) \(:?(?P<weight>\d+)\)(:? -> (?P<children>(:?(:?\w+(, )?))*))?$")
        .unwrap();
    ...
    for caps in re.captures_iter(s) {
        ...
        let children: Vec<Name> = caps.name("children").map(|mat| mat.as_str().split(", ").collect()).unwrap_or_default();
    ...
    fn find_unbalanced(disk: &Disk) -> Result<usize, usize> {
        ...
        for (i,&weight) in results.iter().enumerate() {
            if weight != maj_weight {
                let maj_weight = if i > 1 { results[i-1] } else { results[i+1] };
