...
#[derive(Debug)]
enum Stream { Group(Vec<Stream>), Garbage(u16) }
...
fn garbage_parser(rem : &str) -> IResult<&str, Stream> {
    let (input, l) = delimited(char('<'), fold_many0(in_garbage, u16::default, |a, b| a + b), char('>'))(rem)?;
    Ok((input, Stream::Garbage(l)))
}

fn group_parser(rem : &str) -> IResult<&str, Stream> {
    let (input, v) = delimited(char('{'), separated_list0(char(','), stream_parser), char('}'))(rem)?;
